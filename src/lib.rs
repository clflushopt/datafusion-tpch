    ...
    ($TABLE_FUNCTION_NAME:ident, $TABLE_FUNCTION_SQL_NAME:ident, $GENERATOR:ty, $ARROW_GENERATOR:ty) => {
        ...
        impl $TABLE_FUNCTION_NAME {
            ...
            pub fn table_name() -> &'static str {
                stringify!($TABLE_FUNCTION_SQL_NAME).strip_prefix("tpch_").unwrap_or_else(|| {
                    panic!(
...
impl TpchTables {
    ...
    fn build_and_register_all_tables(&self, scale_factor: f64) -> Result<()> {
        for &suffix in Self::TPCH_TABLE_NAMES {
            // This match block instantiates the correct provider.
            // If provider construction was more complex or varied, other patterns might be needed.
            match suffix {
                "nation" => {
